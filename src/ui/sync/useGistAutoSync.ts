 ...
 * provider other than `"gist"` (`"githubFile"` is reserved but not wired up
 * anywhere yet), or `syncMode` set to `"off"`/`"manual"`.
 *
...
/** The `linkedRemote` shape this hook acts on; the `"githubFile"` arm of the
 *  union is reserved but has no sync implementation to run here yet. */
