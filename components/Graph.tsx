...
import { saveGraphState, loadGraphState } from '../utils/storage';
...
const Graph = ({ project, view, onProjectChange, onViewChange }) => {
  ...
  return (
    <div>
      <h2>Graph</h2>
      <div>
        <button onClick={() => addNode({ id: Date.now(), label: 'New Node' })}>Add Node</button>
        <button onClick={() => addEdge({ id: Date.now(), source: nodes[0]?.id, target: nodes[1]?.id })}>Add Edge</button>
      </div>
      <div>
        {nodes.map((node) => (
          <div key={node.id}>
            <span>{node.label}</span>
            <button onClick={() => removeNode(node.id)}>Remove Node</button>
            <button onClick={() => updateNode({ ...node, label: 'Updated Node' })}>Update Node</button>
          </div>
        ))}
      </div>
      <div>
        {edges.map((edge) => (
          <div key={edge.id}>
            ...
            <button onClick={() => removeEdge(edge.id)}>Remove Edge</button>
            <button onClick={() => updateEdge({ ...edge, label: 'Updated Edge' })}>Update Edge</button>
          </div>
        ))}
      </div>
    </div>
